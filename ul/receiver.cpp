...
#include "network.h"
Receiver::Receiver(int ns, int nr, int file_number, Entity entity, int rank) : rank_(rank) {
  channel_init(ns);
...
auto start = std::chrono::system_clock::now();
    ...
    if (!sys_param->P[rank][rank_]) {
      std::cout << "sender " << rank << "can't comm with receiver " << rank_
                << std::endl;
      return;
    }
    ...
    for (size_t j = 1; j < sys_param->m + 1; ++j) {
      auto it = std::find(sys_param->sij.begin(), sys_param->sij.end(), j);
      if (it == (sys_param->sij).end()) {
#ifdef CORRECT_TEST
        std::cout << "file " << j << " can't decrypt " << std::endl;
#endif
        continue;
      }
      element_set1(tmp);
      element_set1(tmp_down);
      for (auto k : sys_param->sij) {
        if (k == j) continue;
